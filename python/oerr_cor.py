...
def take_ave1d( INFO, otyp=4002, amax=10, range_in=10.0, elv_in=5.0,
   ...
   for ir, ie in zip( range_il, elv_il ):
#       print( ir, ie )
       obs_ = obs[ ( range_i >= ir ) & 
                   ( range_i < ( ir + dr ) ) &
                   ( elv_i >= ie ) &
...
#       print( obs_.shape )
       ...
       for i in range( len( obs_ ) ):
           ...
           da_ = ( azm_i_ - azm_i_[i] ).astype('int32') + amax 
...
def main_az( stime=datetime(2019,9,10,15,0), etime=datetime(2019,9,10,15,0), otyp=4002,
...
for elv_in in elv_inl:
   for range_in in range_inl:
      ...
      main_az( stime=stime, etime=etime, stime0=stime0, otyp=otyp, exp=exp, 
