...
#    x2d_, y2d_ = m_l[0]( lon2d_4, lat2d_4 )
    ...
    levs_u = np.arange( -16, 18, 2)
    levs_u_dif = np.arange( -8, 9, 1 )
    ...
    cmap_t_dif = plt.cm.get_cmap("RdBu_r")
    levs_t_dif = np.arange( -5, 6, 1 )
    ...
    cmap_qv = plt.cm.get_cmap("BuGn")
    cmap_qv.set_over('k', alpha=1.0)
    cmap_qv.set_under('w', alpha=0.0)
    levs_qv = np.arange( 0, 19, 1)

    levs_qv_dif = np.arange( -3, 3.5, 0.5)
    cmap_qv_dif = plt.cm.get_cmap("BrBG")
...
#          fcst3d -= tave
       ...
       var2d_ = var2d[:xlen-1,:ylen-1]*fac
       if i == 0 or i == 3:
          var2d_1 = var2d_
       elif i == 1 or i == 4:
          var2d_2 = var2d_
       else:
          var2d_ = var2d_1 - var2d_2

       cmap.set_under('gray', alpha=1.0)
       cmap.set_over('k', alpha=1.0)
       SHADE = ax.pcolormesh( x2d, y2d*0.001, var2d_,
...
#                  )
       ...
       if i == 1:
          ax.text( 0.5, 1.01, vtime.strftime('Valid at %H:%M:%S UTC %m/%d/%Y') ,
                  va='bottom', 
                  ha='center',
                  transform=ax.transAxes,
                  color='k', fontsize=14, )
    ...
    ofig = "6p_fcst_dif_crs_{0:}_{1:}_cll{2:.3f}_{3:}_{4:}.png".format(  itime.strftime('%m%d%H%M%S'), CRS, cll, nvar1, nvar2 )
    print(ofig)
...
itime1 = datetime( 2019, 8, 24, 15, 30, 0 )
tlev1 = 0
tlev2 = 10
tlev3 = 20

itime2 = datetime( 2019, 8, 24, 15, 25, 0 )
