...
def main( INFO, itime=datetime( 2019, 8, 24, 15, 0, 0), tlev_l=[], 
    ...
    xlab = r'Area-averaged mixing ratio (g m$^{-3}$)'
    ylab = 'Height (km)'
    ...
    for i, ax in enumerate( ax_l ):
       ...
       qc3d = read_fcst_grads_all( INFO, itime=itime, tlev=tlev , FT0=True, nvar='qc' ) 
       qr3d = read_fcst_grads_all( INFO, itime=itime, tlev=tlev , FT0=True, nvar='qr' ) 
       qi3d = read_fcst_grads_all( INFO, itime=itime, tlev=tlev , FT0=True, nvar='qi' ) 
       qs3d = read_fcst_grads_all( INFO, itime=itime, tlev=tlev , FT0=True, nvar='qs' ) 
       qg3d = read_fcst_grads_all( INFO, itime=itime, tlev=tlev , FT0=True, nvar='qg' ) 

       tk3d = read_fcst_grads_all( INFO, itime=itime, tlev=tlev , FT0=True, nvar='t' ) 

       qr1d = np.nanmean( qr3d*afac, axis=(1,2) )
       qc1d = np.nanmean( qc3d*afac, axis=(1,2) )
       qi1d = np.nanmean( qi3d*afac, axis=(1,2) )
       qs1d = np.nanmean( qs3d*afac, axis=(1,2) )
       qg1d = np.nanmean( qg3d*afac, axis=(1,2) )
       ...
       for tk in tk_l:
           ...
           ax.hlines( y=hgt*zfac, xmin=xmin, xmax=xmax, linewidths=1.5, 
                      colors='gray', linestyle='dashed' ) #label=r'{0:}$^\circ$C'.format( tk - tk0 ) )
       ...
       ax.plot( qc1d*fac, cz*zfac, color=cc, linewidth=lw, label=labc )
       ax.plot( qr1d*fac, cz*zfac, color=cr, linewidth=lw, label=labr )
       ax.plot( qi1d*fac, cz*zfac, color=ci, linewidth=lw, label=labi )
       ax.plot( qs1d*fac, cz*zfac, color=cs, linewidth=lw, label=labs )
       ax.plot( qg1d*fac, cz*zfac, color=cg, linewidth=lw, label=labg )
